/*
 *
 * RayTrace Software Package, release 4.beta.  March 29, 2018.
 *
...
public:
	...
	Matrix3x4& ApplyXRotationLeft( double theta );
	Matrix3x4& ApplyXRotationLeft( double costheta, double sintheta );
	Matrix3x4& ApplyYRotationLeft(double theta);
	Matrix3x4& ApplyYRotationLeft(double costheta, double sintheta);
	Matrix3x4& ApplyZRotationLeft(double theta);
	Matrix3x4& ApplyZRotationLeft(double costheta, double sintheta);

	Matrix3x4& ReNormalize();
	VectorR3 Solve(const VectorR3&) const;	// Returns solution

	inline void Transform( VectorR3* ) const;
	inline void Transform( VectorR4* ) const;
	inline void Transform3x3( VectorR3* ) const;
	inline void Transform( const VectorR3& src, VectorR3*  dest ) const;
	inline void Transform( const VectorR4& src, VectorR4*  dest ) const;
...
public:
	...
	AffineMapR3& ApplyXRotationLeft( double theta ) 
									{ Matrix3x4::ApplyXRotationLeft( theta ); return *this; }
	AffineMapR3& ApplyXRotationLeft( double costheta, double sintheta ) 
									{ Matrix3x4::ApplyXRotationLeft( costheta, sintheta ); return *this; }
...
public:
	static const RotationMapR3 Identity;
...
public:
	...
	RotationMapR3& Set( const Quaternion& );			// Defined in Quaternion.cpp
	...
	RotationMapR3& SetFromEulersYX( double rotationAngleY, double azimuth );	// Intended for cameras (only two angles used)

	RotationMapR3& operator*= (const RotationMapR3& );	// Matrix product
	void RightMultiplyByTranspose( const RotationMapR3& M ) { Matrix3x3::RightMultiplyByTranspose(M); }
	void LeftMultiplyBy( const RotationMapR3& M ) { Matrix3x3::LeftMultiplyBy(M); }
	void LeftMultiplyByTranspose( const RotationMapR3& M ) { Matrix3x3::LeftMultiplyByTranspose(M); }
	...
	double ToAngle() const;  // returns angle of rotation (-PI,PI] range (from atan2)
...
public:
	...
	RigidMapR3& SetRotationPart( const Quaternion& );		// Still needs to be written (See RotationMapR3.Set.)
	...
	RigidMapR3& ApplyTranslationRight( const VectorR3& u )
								{Matrix3x4::ApplyTranslationRight( u ); return *this;}
	RigidMapR3& ApplyXRotationLeft( double theta ) 
								{ Matrix3x4::ApplyXRotationLeft( theta ); return *this; }
	RigidMapR3& ApplyXRotationLeft( double costheta, double sintheta ) 
								{ Matrix3x4::ApplyXRotationLeft( costheta, sintheta ); return *this; }
...
// Left-multiply with a rotation around the x-axis.
inline Matrix3x4& Matrix3x4::ApplyXRotationLeft( double theta ) 
{
	double costheta = cos(theta);
	double sintheta = sin(theta);
	return ApplyXRotationLeft( costheta, sintheta );
}

// Left-multiply with a rotation around the x-axis.
inline Matrix3x4& Matrix3x4::ApplyXRotationLeft( double costheta, double sintheta ) 
{
	double tmp;
	tmp = costheta*m21-sintheta*m31;
	m31 = costheta*m31+sintheta*m21;
	m21 = tmp;

	tmp = costheta*m22-sintheta*m32;
	m32 = costheta*m32+sintheta*m22;
	m22 = tmp;

	tmp = costheta*m23-sintheta*m33;
	m33 = costheta*m33+sintheta*m23;
	m23 = tmp;

	tmp = costheta*m24-sintheta*m34;
	m34 = costheta*m34+sintheta*m24;
	m24 = tmp;
...
// Left-multiply with a rotation around the y-axis.
inline Matrix3x4& Matrix3x4::ApplyZRotationLeft(double theta)
{
	double costheta = cos(theta);
	double sintheta = sin(theta);
	return ApplyZRotationLeft(costheta, sintheta);
}

inline Matrix3x4& Matrix3x4::ApplyZRotationLeft( double costheta, double sintheta ) 
{
	double tmp;
	tmp = costheta * m11 - sintheta * m21;
	m21 = costheta * m31 + sintheta * m11;
	m11 = tmp;

	tmp = costheta * m12 - sintheta * m22;
	m22 = costheta * m32 + sintheta * m12;
	m12 = tmp;

	tmp = costheta * m13 - sintheta * m23;
	m23 = costheta * m33 + sintheta * m13;
	m13 = tmp;

	tmp = costheta * m14 - sintheta * m24;
	m24 = costheta * m34 + sintheta * m14;
	m14 = tmp;
