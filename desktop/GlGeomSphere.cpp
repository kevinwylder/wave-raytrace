...
void GlGeomSphere::LoadBufferData() {
    ...
    float *tempStoref = new float[GetNumVertices() * 8];
    ...
    for (int i = 0; i <= numSlices; i++) {
        ...
        float sTex = ((float)i) / (float)numSlices;
        float costheta = cos(theta);
        float sintheta = sinf(theta);
        for (int j = 1; j < numStacks; j++) {
            float tTex = ((float)j) / (float)(numStacks);
            float phi = tTex * (float)PI;
            ...
            if (UseTexCoords()) {
                tempStoref[s] = (i==numSlices) ? 1.0f :  sTex;   // s texture coordinate
                tempStoref[s + 1] = tTex;                       // texture coordinate
                s += 2;
            }
        }
    }
    glBindVertexArray(theVAO);
    glBindBuffer(GL_ARRAY_BUFFER, theVBO);
    glBufferData(GL_ARRAY_BUFFER, 8*GetNumVertices()*sizeof(float), tempStoref, GL_STATIC_DRAW);
